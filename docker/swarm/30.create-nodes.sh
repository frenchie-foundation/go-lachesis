...
do
  ...
  docker $SWARM service create \
   ...
   ${REGISTRY_HOST}/lachesis:${TAG} --nousb \
    ...
    --http --http.addr="0.0.0.0" --http.port=${RPCP} --http.vhosts="*" --http.corsdomain="*" --http.api="eth,debug,admin,web3,personal,net,txpool,fren,sfc" \
    --ws --ws.addr="0.0.0.0" --ws.port=${WSP} --ws.origins="*" --ws.api="eth,debug,admin,web3,personal,net,txpool,fren,sfc" \
